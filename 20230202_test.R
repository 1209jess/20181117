...
#select the column needed
df4_2 <- df3_2[1:5,c(2,3,10,14)]

#plot chart
p2 <- ggplot(df4_2, aes(x = Fold, y = reorder(Term, log_pvalue_2), size = Count,colour = log_pvalue_2)) +
  geom_point()+
  theme_bw()+
  labs(title="ALA_48hr_cluster(-357)_CC", x="Fold Enrichment", y="Term", size="# genes", colour="-log(Pvalue)")+
  theme_bw()+
  scale_colour_gradient(guide=guide_colourbar(reverse = FALSE),
                        low="blue", high="red")+
  scale_y_discrete(labels = function(y) str_wrap(y, width = 45))+
...
#read data
df_3 <- read.delim("ALA_48hr_cluster(-357)_GOTERM_MF.txt")
...
#select the column needed
df4_3 <- df3_3[1:5,c(2,3,10,14)]

#plot chart
p3 <- ggplot(df4_3, aes(x = Fold, y = reorder(Term, log_pvalue_3), size = Count,colour = log_pvalue_3)) +
  geom_point()+
  theme_bw()+
  labs(title="ALA_48hr_cluster(-357)_MF", x="Fold Enrichment", y="Term", size="# genes", colour="-log(Pvalue)")+
  scale_colour_gradient(guide=guide_colourbar(reverse = FALSE),
                        low="blue", high="red")+
  scale_y_discrete(labels = function(y) str_wrap(y, width = 45))+
  scale_x_continuous(expand = c(0.1, 0))+
...
#read data
df_4 <- read.delim("ALA_48hr_cluster(-357)_KEGG.txt")
...
#select the column needed
df4_4 <- df3_4[1:5,c(2,3,10,14)]

#plot chart
p4 <- ggplot(df4_4, aes(x = Fold, y = reorder(Term, log_pvalue_4), size = Count, colour = log_pvalue_4)) +
  geom_point()+
  theme_bw()+
  labs(title="ALA_48hr_cluster(-357)_KEGG", x="Fold Enrichment", y="Term", size="# genes", colour="-log(Pvalue)")+
  scale_colour_gradient(guide=guide_colourbar(reverse = FALSE),
                        low="blue", high="red")+
  scale_y_discrete(labels = function(y) str_wrap(y, width = 45))+
  scale_x_continuous(expand = c(0.1, 0))+
